...
//just the ramp down half of the triangular travel traj
void TrajBuilder::build_ramp_down_travel_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
    ...
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
    double phi = convertPlanarQuat2Psi(start_pose.pose.orientation);
    geometry_msgs::PoseStamped end_pose;
    end_pose.pose.position.x = x_start = 0.5 * cos(phi);
    end_pose.pose.position.y = y_start = 0.5 * sin(phi);
    ...
    double dx = x_end - x_start;
    double dy = y_end - y_start;
    vec_of_states.clear(); //reset the trajectory
    build_ramp_down_travel_traj(start_pose, end_pose, vec_of_states);
}
